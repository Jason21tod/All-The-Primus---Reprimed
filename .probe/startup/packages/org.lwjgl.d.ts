declare module "org.lwjgl.glfw.GLFWVidMode" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $GLFWVidMode$$Type = ($GLFWVidMode);
}

declare module "org.lwjgl.CLongBuffer" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $CLongBuffer$$Type = ($CLongBuffer);
}

declare module "org.lwjgl.system.CallbackI" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $CallbackI$$Type = ($CallbackI);
}

declare module "org.lwjgl.glfw.GLFWVidMode$Buffer" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $GLFWVidMode$Buffer$$Type = ($GLFWVidMode$Buffer);
}

declare module "org.lwjgl.stb.STBIWriteCallbackI" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $STBIWriteCallbackI$$Type = ($STBIWriteCallbackI | ((arg0: long, arg1: long, arg2: integer) => void));
}

declare module "org.lwjgl.stb.STBTTFontinfo" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $STBTTFontinfo$$Type = ($STBTTFontinfo);
}

declare module "org.lwjgl.system.libffi.FFIType" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $FFIType$$Type = ($FFIType);
}

declare module "org.lwjgl.system.libffi.FFIType$Buffer" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $FFIType$Buffer$$Type = ($FFIType$Buffer);
}

declare module "org.lwjgl.PointerBuffer" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $PointerBuffer$$Type = ($PointerBuffer);
}

declare module "org.lwjgl.stb.STBTTFontinfo$Buffer" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $STBTTFontinfo$Buffer$$Type = ($STBTTFontinfo$Buffer);
}

declare module "org.lwjgl.system.Pointer$Default" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $Pointer$Default$$Type = ($Pointer$Default);
}

declare module "org.lwjgl.system.libffi.FFICIF" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $FFICIF$$Type = ($FFICIF);
}

declare module "org.lwjgl.system.Pointer" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $Pointer$$Type = ($Pointer | (() => long));
}

declare module "org.lwjgl.system.Struct" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $Struct$$Type = ($Struct);
}

declare module "org.lwjgl.system.libffi.FFICIF$Buffer" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $FFICIF$Buffer$$Type = ($FFICIF$Buffer);
}

declare module "org.lwjgl.system.StructBuffer" {
import { $Struct, $Struct$$Type } from "org.lwjgl.system.Struct"

/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $StructBuffer$$Type<T extends $Struct = $Struct, SELF extends $StructBuffer<T, SELF> = $StructBuffer<T, SELF>> = ($StructBuffer<T, SELF>);
}

declare module "org.lwjgl.system.NativeResource" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $NativeResource$$Type = ($NativeResource | (() => void));
}

declare module "org.lwjgl.system.CustomBuffer" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $CustomBuffer$$Type<SELF extends $CustomBuffer<SELF> = $CustomBuffer<SELF>> = ($CustomBuffer<SELF>);
}

declare module "org.lwjgl.system.MemoryStack" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $MemoryStack$$Type = ($MemoryStack);
}

declare module "org.lwjgl.system.Struct$StructValidation" {
export {} // Mark the file as a module, do not remove unless there are other import/exports!
/** Use `Internal.{Type}` and `Internal.{Type}_` for referencing this type in JS file */
export type $Struct$StructValidation$$Type = ($Struct$StructValidation | ((arg0: long) => void));
}

